...
def Clear_terminal():
	if "Linux" in platform.platform():
		...
		print "[-] Sorry, This script runs only on Windows and Linux platforms"
		sys.exit()
...
def Print_banner():
	print '''
 ____            _                                     
|  _ \ ___  _ __| |_ ___  ___ __ _ _ __    _ __  _   _ 
| |_) / _ \| '__| __/ __|/ __/ _` | '_ \  | '_ \| | | |
|  __/ (_) | |  | |_\__ \ (_| (_| | | | |_| |_) | |_| |
|_|   \___/|_|   \__|___/\___\__,_|_| |_(_) .__/ \__, |
                                          |_|    |___/  Coded by: Sha2ow_M4st3r'''


def Interface(Victime):
	print "\n#---------------------------------------------#"
	print "[+] Scanning start at:", time
	print "#---------------------------------------------#"
	print " Victime:",Victime
	print "+---------------------------------------------+"
	print "[Port]\t[Services]\t[Status]"
	print "+---------------------------------------------+"
...
,1759,1789,1812,1813,1911,1985,1986,1997,2049,2053,2102,2103,2104,2105,2401,2430,2430,2431,2600,2601,2602,2603,2604,2605,2606,2809,3130,3306,3346,4011,4321,4444,5002,5308,5431,5999,6000,7000,7001,7002,
7003,7004,7005,7006,7007,7008,7009,9876,10080,11371,11720,13720,13721,13722,13724,13782,13783,22273,26000,26208,33434]



	# All ports services
	Services = {1:'tcpmux', 2:'compressnet', 3:'compressnet', 4:'sfs', 5:'rje', 7:'echo', 8:'trojan - Ping Attack',
...
181:'unify',182:'audit',183:'ocbinder',184:'ocserver',185:'remote-kis',186:'kis',187:'aci',188:'mumps',5431:"park-agent",
...
13718:'VeritasNetbackup',13720:'VeritasNetbackup',13721:'VeritasNetbackup',13722:'VeritasNetbackup',13782:'VeritasNetbackup'}


	try:
		for Checking_port in range(len(Ports)):
			Server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
			Server.settimeout(0.5)
			Response = Server.connect_ex((IP_addr,Ports[Checking_port]))
			Server.close()

			if Response == 0:
				print " ",Ports[Checking_port],"\t",Services[Ports[Checking_port]], "\t\tOpen"

	except socket.error as msg:
		print "[-] Socket error:",str(msg)
		sys.exit()
	except KeyboardInterrupt:
		print "[-] Script stopped, You press the CTRL+C"
		print "[+] Now time:", time
		sys.exit()
	except:
		print "[!] Unknown error"
...
def using_all_functions():
	Parser = argparse.ArgumentParser(epilog="""Port scanner:\n
	This script gives you the ability to view open
	ports for penetration test.""", usage="Portscan.py -i <victime-ip>""")
	Parser.add_argument("-i", help="Use this switch for victime ip address", nargs=1, type=str)
	Args = Parser.parse_args()

	if Args.i == None:
		print "[!] Usage: Portscan.py -i <victime-ip> "
		sys.exit()
	else:
		Clear_terminal()
		Print_banner()
